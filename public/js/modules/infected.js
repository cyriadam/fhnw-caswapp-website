...
const distance = (coord1={x:0,y:0}, coord2={x:0,y:0}) => Math.sqrt(Math.pow(Math.abs(coord1.x-coord2.x), 2)+Math.pow(Math.abs(coord1.y-coord2.y), 2));
...
let Game = (x, y, width, height) => {
  ...
  return {
    ...
    getBoundaries : () => { return {x:Lamba.fst(boundaries), y:Lamba.snd(boundaries)} },
    getCellSize : () => { return {width:Lamba.fst(cellSize), height:Lamba.snd(cellSize)} },
...
const Controler = (game) => {
  ...
  const vidal = [Cell('cell', ()=>2+random(3), 1, 0, -1, 1, -1, -1), Cell('virus', ()=>2+random(3), 0.8, 0, -1, 3, 1, -1), Cell('anticorps', ()=>5, 0.5, 2, 20, -1, 0, 0.5)];
  ...
  const createCell = (cellType) => {                                    
    ...
    const cell = {
      ...
      setInfected: (byCell) => {
        if(type=='cell'&&byCell.getType()=='virus') {                               // cell became a virus      
        ...
        } else if(type=='virus'&&byCell.getType()=='anticorps') {                   // anticorps kill the virus
      ...
      click: () => {
        if(type=='virus') {
          switch(cure) {
            case 0 : energyLevel.setValue(energyLevel.getValue()-1); break;     // reduce energyLevel
            case 1 : energyLevel.setValue(0); break;                            // kill the virus
            case 2 : lifeTime=lifeTime<0?game.getPoison():lifeTime>game.getPoison()?game.getPoison():lifeTime; break;        // poisoning the virus 
            case 3 : speed.setValue(Math.max(1, speed.getValue()-1)); break;    // reduce the speed
            case 4 : speed.setValue(speed.getValue()+1); break;                 // increase the speed
          }
        }
      ...
      allAnticorps.forEach(anticorps=>{
        if(anticorps.getIA()>0) {
          ...
          allVirus.forEach(virus=>{
            ...
            else if(anticorps.getIA()==2) {   // search closest way moving out of the screen bounbaries
              let caseId=1;   
              [-1, 0, 1].forEach(i=>{
                [-1, 0, 1].forEach(j=>{
                  ...
                  if(!virusCoordinates||d<=getDistance(virusCoordinates)) virusCoordinates=Coordinates(x, y, d, `target virus=[${virus.getId()}] (case${caseId})`);
                  caseId++;
                })
              });
            }
  ...
  return {
    ...
    click: (coord) => {
      let min=cellList.getList().map((cell, i)=> { return {i, d:distance(coord, {x:cell.x, y:cell.y})}}).reduce((acc, curr) => (curr.d<acc.d?curr:acc), {i:-1, d:100});
...
export const View = (canvas, addCellBtn, addVirusBtn, addAnticorpsBtn, nbOfCells, nbOfVirus, score, nbOfBullets, cellImg, virusImg, anticorpsImg) => {
  ...
  const renderBoard = () => {  
    (Log.getLogLevel()===Log.LEVEL_DEBUG)?(()=>{ // then 
      for(let i=0; i<=game.getBoundaries().x; i++) drawLine(context, ...Object.values(convertToCanvas(i, 0)), ...Object.values(convertToCanvas(i, game.getBoundaries().y)));
      for(let j=0; j<=game.getBoundaries().y; j++) drawLine(context, ...Object.values(convertToCanvas(0, j)), ...Object.values(convertToCanvas(game.getBoundaries().x, j)));
    })():(()=>{ // else
    })();
    ...
    controler.getCells().forEach(cell => {
      let {x, y} = convertToCanvas(cell.x, cell.y);
      let {x:cellRadius} = convertToCanvas(cell.getRadius());
      context.drawImage(cell.getType()=='virus'?virusImg:cell.getType()=='anticorps'?anticorpsImg:cellImg, x-cellRadius, y-cellRadius, 2*cellRadius, 2*cellRadius);
