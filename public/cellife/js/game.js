...
const Player = (playerId, coord, direction, stateValue, score, nbBullets) => { 
    ...
    return {
      ...
      score: () => score,
      nbBullets: () => nbBullets,
      update: (coordValue, directionValue, stateValue, scoreValue, nbBulletsValue) => { coord=coordValue; direction=directionValue; state.setValue(stateValue); score=scoreValue; nbBullets=nbBulletsValue; }
    }
  };
...
const GameController = (socket, hallOfFameController, partyController, menuController) => {
  ...
  return {
    onAddPlayers: players.onAdd,
      newParty: (nbPlayers, partyName) => partyController.newParty(nbPlayers, partyName, user.getName(), res => {
      ...
      joinParty: (partyId) => partyController.joinParty(partyId, user.getName(), res => {
          if (res.error) { alert(`Error: ${res.error}`); return; }
          user.setId(res.data.playerId);
          if (gameState.getValue() != 'locked') {
              gameState.setValue('join');
      ...
      leaveParty: () => partyController.leaveParty(res => {
          if (res.error) { alert(`Error: ${res.error}`); return; }
...
const GameView = (gameController, hallOfFameController, rootElt) => {
  ...
  placeholderExpend.onChange(expend=>{
    expend?document.getElementById("placeholder-gameboard").classList.add('expend'):document.getElementById("placeholder-gameboard").classList.remove('expend');
  ...
  gameController.onUserNbBulletsChange(nbBullets=> nbBulletsElt.innerHTML=Array.from({length:nbBullets}).reduce((p, c) => p+'<ion-icon name="skull-outline"></ion-icon>', ''));
  ...
  hallOfFameController.hallOfFame.onChange(scores=> highScoreElt.innerHTML=scores.reduce((acc, curr) => curr.score.getValue()>acc?curr.score.getValue():acc, 0));
