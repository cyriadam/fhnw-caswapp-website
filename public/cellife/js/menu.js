...
const MenuController = (menuItemConstructor) => {
    ...
    const addMenuItem = template => {
        ...
        item.selected.onChange(selected=> { if(selected) closeOtherMenuItems(item.id)});
    ...
    const closeOtherMenuItems = (id) => {
        listMenuItems.getList().filter(menuItem => menuItem.id != id).forEach(menuItem => menuItem.selected.setValue(false));
    }
