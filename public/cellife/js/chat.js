...
const ChatItemModel = (data) => {
    ...
    return {
        ...
        emit,
        text,
        toString: () => `chatItem={id=[${id}], userName=[${userName.getValue()}], createdAt=[${createdAt.getValue()}], emit=[${emit.getValue()}], text=[${text.getValue()}]}`,
    }
};
...
const ChatController = (chatItemConstructor, nbMaxItems=-1) => {
    ...
    listChatItems.onAdd(item => { if(listChatItems.count()>nbMaxItems) listChatItems.del(listChatItems.getList()[0]); });
